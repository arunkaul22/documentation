...
def check_redirects_format(file, lines, options=None):
    ...
    if file.startswith('redirects/'):  # Do no check text files that are not in the redirects folder
        for lno, line in enumerate(lines):
            ...
            if not match_res:
                yield lno + 1, 'invalid content in redirects file'

@sphinxlint.checker('.txt')
def check_redirects_target(file, lines, options=None):
    """Check that files targeted by redirect rules exist."""
    if file.startswith('redirects/'):  # Do no check text files that are not in the redirects folder
        def _get_version_of_redirects_file(filename):
            if filename.startswith('redirects/'):
                filename = filename[10:]
            if filename.startswith('saas'):
                filename = filename[5:]
            return float(filename[:-4]) # drop ".txt"
        ...
        for lno, line in enumerate(lines):
            ...
            if match_res:
                ...
                if not file_.is_file():
                    yield lno + 1, f"redirects rule targets unexisting file {new_path}"
